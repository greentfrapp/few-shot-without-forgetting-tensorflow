...
class CNN_miniimagenet(BaseModel):
	...
	def __init__(self, name, num_classes, input_tensors=None, mode='fewshot', use_attention=False):
	...
	def build_model(self, input_tensors=None):
		...
		if self.mode == 'pretrain':
			...
			self.logits = tf.matmul(feature_extractor.output, classifier_weights, transpose_b=True)
			self.logits = scale * self.logits

			self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels, logits=self.logits))
			self.optimize = tf.train.AdamOptimizer(learning_rate=1e-4).minimize(self.loss)
			# self.optimize = tf.train.MomentumOptimizer(learning_rate=self.lr, momentum=0.9, use_nesterov=True).minimize(self.loss)
			...
			if self.use_attention:
				with tf.variable_scope('attention'):
					fs_weights = tf.get_variable(
						shape=(1, 1, 5*5*32),
						name='fs_weights',
						dtype=tf.float32,
					)
					class_weights = class_weights * fs_weights
			...
			if self.use_attention:
				...
				self.optimize = tf.train.AdamOptimizer(learning_rate=1e-4).minimize(self.loss, var_list=att_vars)
